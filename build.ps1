...
# platforms (same lesson carried over from Chronicle_Scrobbler's build.ps1).
...
# Copy lib/, python/, and resources/ (excluding __pycache__)
Copy-Item "$projectRoot\lib" "$sourcePath\lib" -Recurse -Force
Copy-Item "$projectRoot\python" "$sourcePath\python" -Recurse -Force
