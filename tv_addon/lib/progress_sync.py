...
feature that mechanism replaces). Chronicle_Scraper's two addons (this one and
the movies addon at the repo root) have no runtime dependency on
Chronicle_Scrobbler or each other, so the logic is duplicated here rather than
imported -- same as chronicle_client.py is already duplicated between them.
...
def lookup_episode_state(tvshowid, season, episode):
    ...
    below treats a missing kodi_item as "nothing to reconcile against, just
    push whatever Chronicle has"."""
    if tvshowid is None:
        return None
    request = {
        ...
        'params': {
            'tvshowid':   tvshowid,
            'properties': _STATE_PROPERTIES + ['season', 'episode'],
        },
    }
    try:
        response = json.loads(xbmc.executeJSONRPC(json.dumps(request)))
    except Exception as exc:
        log.warning('lookup_episode_state(tvshowid={0}): failed: {1}'.format(tvshowid, exc))
        return None
...
def resolve_watched_direction(chronicle_watched, chronicle_watched_at, kodi_item):
    ...
    this: Chronicle clears ResumePositionPercent/ResumeUpdatedAt to null the moment an item
    is marked watched (nothing left to "resume"), so a completed episode carries no signal
    for that function to compare -- it correctly returns (None, None) and nothing happens.
    This function compares Chronicle's own IsWatched/LastWatchedAt (never cleared, see
