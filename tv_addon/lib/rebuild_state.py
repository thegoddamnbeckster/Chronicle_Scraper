...
# Deliberately NOT special://profile/addon_data/{addon_id}/... -- that path is
# scoped per addon id, but Chronicle Scraper is split into two separate addon
# packages (script.chronicle.scraper.movie, script.chronicle.scraper.tv;
# Kodi resolves a scraper invocation by addon id alone, so movies/TV can't
# share one addon -- see the .movie/.tv split itself). nfo_rebuild.py's
# mark_started()/mark_finished() run from the movie addon's process;
# get_details()/get_episode_details() checking is_active() can run from
# either addon's process. special://temp/ is the one location both addon
# ids can read and write without depending on each other's addon_data
# folder existing or being named a specific thing.
_STATE_PATH = 'special://temp/chronicle_scraper/rebuild_active.json'
