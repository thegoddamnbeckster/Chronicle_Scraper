...
class TestDiffShowText(unittest.TestCase):
    ...
    def test_year_is_never_written_because_kodi_has_no_such_parameter(self):
        # SetTVShowDetails rejects the whole call for an unknown parameter; a show's year follows its
        # premiered date, which is corrected instead.
        self.assertNotIn('year', diff_show_text(_kodi(year=2017), {'year': 2026}))
        self.assertEqual(diff_show_text(_kodi(year=2017), {'year': 2026, 'premiered': '2026-01-18'}),
                         {'premiered': '2026-01-18'})
